...
bot.hears('Заказы обуви с POIZON', async (ctx) => {
    ...
    await ctx.reply('Укажите стоимость обуви в юанях (¥), чтобы я рассчитал стоимость в USD с доставкой в Минск.');
});

bot.hears('Заказы иных товаров с POIZON', async (ctx) => {
    ctx.session.waitingForOtherPrice = true; // Устанавливаем флаг ожидания цены
    await ctx.reply('Укажите стоимость товара в юанях (¥), чтобы я рассчитал стоимость в USD с доставкой в Минск.');
});
